...
function renderCalculator() {
  ...
  breakdownRows.replaceChildren(
    ...questions.map((question, index) => {
      ...
      tr.innerHTML = `
        <td>${question}</td>
        <td>${bRow.want.desired || "doesn't care"}</td>
        <td>${bRow.answer.actual}</td>
        <td class="points">${percent(bRow.points)}</td>
        <td>${aRow.want.desired || "doesn't care"}</td>
...
function renderTopMatches() {
  ...
  topMatches.replaceChildren(
    ...ranked.map((match) => {
      ...
      item.innerHTML = `<strong>${match.a.name}</strong> + <strong>${match.b.name}</strong>: ${percent(match.final)}`;
      return item;
    }),
  );
}
